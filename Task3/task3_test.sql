CREATE VIEW lesson_per_month AS
    ...
    count(lesson_type) FILTER (WHERE lesson_type = 'induvidual') as individual_lesson, 
...
CREATE VIEW student_nr_of_siblings
    ...
    SELECT nr_of_siblings, count(1) as students_with_nr_of_siblings FROM (SELECT count(sibling.student_id) FROM student LEFT JOIN sibling ON student.student_id = sibling.student_id  GROUP BY student.student_id) as nr_of_siblings GROUP BY nr_of_siblings ORDER BY nr_of_siblings ASC;
...
CREATE VIEW student_nr_of_siblings
    ...
    SELECT lesson.instructor_id, count(lesson.instructor_id) FROM lesson LEFT JOIN instructor ON instructor.instructor_id = lesson.instructor_id WHERE EXTRACT(month FROM time) = EXTRACT(month FROM now()) AND EXTRACT(year FROM time) = EXTRACT(year FROM now()) GROUP BY lesson.instructor_id HAVING count(lesson.instructor_id) > 1;
    
    SELECT lesson.instructor_id, count(lesson.instructor_id) FROM lesson LEFT JOIN instructor ON instructor.instructor_id = lesson.instructor_id WHERE EXTRACT(month FROM time) = '10' AND EXTRACT(year FROM time) = '2021' GROUP BY lesson.instructor_id HAVING count(lesson.instructor_id) > 1;
